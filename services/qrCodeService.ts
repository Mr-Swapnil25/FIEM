...
export const generateQRCodeData = (
  ...
  // Add a simple signature for verification
  const signature = btoa(`${eventId}:${ticketId}:${userId}`).slice(0, 10);
 ...
 * Uses canvas-based generation for browser compatibility
 */
...
export const validateQRCode = (qrData: string): QRValidationResult => {
  try {
    ...
    // Verify signature
    const expectedSignature = btoa(`${data.eventId}:${data.ticketId}:${data.userId}`).slice(0, 10);
    if (data.signature && data.signature !== expectedSignature) {
      return { valid: false, error: 'Invalid ticket signature' };
    }
    
    // Check if ticket is not too old (valid for 1 year)
