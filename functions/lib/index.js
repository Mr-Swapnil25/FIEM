...
exports.checkApiRateLimit = exports.rateLimitAlerts = exports.cleanupRateLimits = exports.cspReport = exports.rateLimitedBooking = exports.rateLimitedAuth = void 0;
 ...
 */
async function checkRateLimit(identifier, endpoint, config) {
    const now = Date.now();
    const docRef = db.collection('rateLimits').doc(`${endpoint}:${identifier}`);
    return db.runTransaction(async (transaction) => {
        const doc = await transaction.get(docRef);
        let entry = doc.exists
            ? doc.data()
 ...
 */
exports.rateLimitedAuth = functions.https.onRequest(async (request, response) => {
    // CORS headers
 ...
 */
exports.rateLimitedBooking = functions.https.onRequest(async (request, response) => {
    // CORS headers
    response.set('Access-Control-Allow-Origin', '*');
    response.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    response.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
 ...
 */
exports.cspReport = functions.https.onRequest(async (request, response) => {
    // Only accept POST
    if (request.method !== 'POST') {
 ...
 */
exports.rateLimitAlerts = functions.pubsub
 ...
 */
exports.checkApiRateLimit = functions.https.onCall(async (data, context) => {
    ...
    const result = await checkRateLimit(identifier, endpoint, config);
    if (!result.allowed) {
        throw new functions.https.HttpsError('resource-exhausted', `Rate limit exceeded. Retry after ${result.retryAfter} seconds.`, { retryAfter: result.retryAfter });
    }
